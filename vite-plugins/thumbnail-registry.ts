...
const defaultOptions: ThumbnailRegistryOptions = {
	...
	outputFile: '.svelte-kit/generated/thumbnail-registry.ts',
...
export function thumbnailRegistryPlugin(options: Partial<ThumbnailRegistryOptions> = {}): Plugin {
	...
	return {
		...
		async configResolved() {
			// Generate registry early in the Vite lifecycle
			await generateThumbnailRegistry(opts);
		},
		async buildStart() {
			// Ensure registry exists before build
			await generateThumbnailRegistry(opts);
		},
...
function generateRegistryCode(thumbnails: string[]): string {
	const imports = thumbnails.map((thumbnail, index) => {
		...
		return {
			plain: `import ${varName} from '../../src/lib/assets/${thumbnail}';`,
			enhanced: `import ${varNameEnhanced} from '../../src/lib/assets/${thumbnail}?enhanced';`,
			varName,
			varNameEnhanced,
			path: thumbnail
		};
...
// This file is cached in .svelte-kit/generated/ and not committed to git

import type { ThumbnailEntry, IThumbnailRegistry } from '../../src/lib/articles/thumbnail-types.js';
 ...
 */
export class GeneratedThumbnailRegistry implements IThumbnailRegistry {
